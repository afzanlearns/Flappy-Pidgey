/**
 * Complete Kanto Pokédex — all 151, real names and types.
 * `tier` is used only for flavor (catch animation color accents, not mechanics)
 * since catching is single-tap and not stat-based in this build.
 */
const POKEDEX = [
...
// Legendaries/mythicals get a slightly lower weight so they feel special —
// still possible, just rarer in the random draw pool.
const LEGENDARY_IDS = new Set([144, 145, 146, 150, 151]);

function getRandomPokemonId() {
  // Weighted pool: legendaries should feel rare (~3% total encounter rate),
  // not just "slightly less common" — so we exclude them from the normal
  // uniform draw and only grant them via the explicit low-probability roll.
  const roll = Math.random();
  if (roll < 0.03) {
    const legendaries = Array.from(LEGENDARY_IDS);
    return legendaries[Math.floor(Math.random() * legendaries.length)];
  }
  // Uniform random across the 146 non-legendary IDs
  const nonLegendaryIds = [];
  for (let i = 1; i <= 151; i++) {
    if (!LEGENDARY_IDS.has(i)) nonLegendaryIds.push(i);
  }
  return nonLegendaryIds[Math.floor(Math.random() * nonLegendaryIds.length)];
}
...
const TYPE_COLORS = {
  Normal: "#A8A878", Fire: "#FF8A50", Water: "#4F9FFF", Electric: "#FFD23F",
  Grass: "#5FBD5F", Ice: "#7FE0E0", Fighting: "#D85A4A", Poison: "#B060D0",
  Ground: "#E0B86A", Flying: "#9FA8FF", Psychic: "#FF6FA5", Bug: "#A8C83A",
  Rock: "#C8B26A", Ghost: "#7060A0", Dragon: "#6F5FE0",
};
